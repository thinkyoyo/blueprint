...
<template>
  <!-- 菜单栏 -->
  <div class="my-side-bar" :style="sidebarStyle">
      ...
      <template v-for="(item, index) in listData">
        <template v-if="!item.hidden">
          <a-sub-menu v-if="item.children?.length" :key="`${index}`">
            <template #icon>
              <div>
                <my-icon v-if="item.meta.antIcon" :name="item.meta.antIcon" />
              </div>
            </template>
            <template #title>
              {{ item.meta.title }}
    ...
    <div class="menu-fold">
      <div class="icon" @click="onCollapsed">
        <my-icon :name="isCollapsed ? 'MenuUnfoldOutlined' : 'MenuFoldOutlined'" />
      </div>
    </div>
  </div>
</template>
